...
BOOLEAN HandleCPUID(PGUEST_REGS state)
{
	...
	// Here we may implement our BPKNOCK Server !
	DbgPrint("[*] CPUID RAX=0x%llx RCX=0x%llx \n", state->rax, state->rcx);
	if (state->rcx == 0x73317331)
	{
		DbgPrint("Server MAGIC 0x73317331; return 0x13371337 ! \n");
		cpu_info[0] = 0x13371337;
	}
